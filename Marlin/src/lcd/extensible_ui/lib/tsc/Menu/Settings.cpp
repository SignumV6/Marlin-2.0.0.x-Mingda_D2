...
void menuInfo(void)
{
  ...
  const char* firmware_from = "based on: Marlin " CONFIGURATION_D_VERSION; 
  const char* autor = "Autor: " STRING_CONFIG_H_AUTHOR;  // GST
  ...
  uint16_t FF_X = (LCD_WIDTH_PIXEL - GUI_StrPixelWidth((uint8_t *)firmware_from))/2; //GST
  uint16_t AU_X = (LCD_WIDTH_PIXEL - GUI_StrPixelWidth((uint8_t *)autor))/2; //GST
  ...
  uint16_t  startF = min(HW_X, FF_X);  // GST
  ...
  GUI_DispString(startF, centerY + BYTE_HEIGHT, (uint8_t *)firmware_from); // GST
  // Space-Line between this two Lines
  GUI_DispString(startY, centerY + 3 * BYTE_HEIGHT, (uint8_t *)autor); // GST
  GUI_DispString(startPT, centerY + 4 * BYTE_HEIGHT, (uint8_t *)printtime); // GST
  GUI_DispString(startLP, centerY + 5 * BYTE_HEIGHT, (uint8_t *)longestprint); // GST
  GUI_DispString(startP, centerY + 6 * BYTE_HEIGHT, (uint8_t *)prints); // GST
  GUI_DispString(startFI, centerY + 7 * BYTE_HEIGHT, (uint8_t *)filament); // GST
