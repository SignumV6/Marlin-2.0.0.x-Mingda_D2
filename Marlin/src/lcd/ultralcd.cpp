...
// All displays share the MarlinUI class
  ...
  constexpr int MAX_TMESSAGE_LENGTH = 1024;
#endif
...
*/
    #if ENABLED(EXTENSIBLE_UI)
      ExtUI::onTrammingChanged(tramming_message);
    #endif
  }
  // GST
  ...
  //GST 
  void MarlinUI::set_t_status(const char * const message, const bool persist) {
    if (alert_level) return;
    ...
    //  If length of supplied UTF8 string is greater than
    // our buffer size, start cutting whole UTF8 chars
    while ((pend - message) > MAX_TMESSAGE_LENGTH) {
      --pend;
      while (!START_OF_UTF8_CHAR(*pend)) --pend;
    };
    ...
    // At this point, we have the proper cut point. Use it
    uint8_t maxLen = pend - message;
    strncpy(tramming_message, message, maxLen);
  ...
  // GST
    void MarlinUI::set_t_status_P(PGM_P const message, int8_t level) {
    if (level < 0) level = alert_level = 0;
    if (level < alert_level) return;
    alert_level = level;
    ...
    // At this point, we have the proper cut point. Use it
    uint8_t maxLen = pend - message;
    strncpy_P(tramming_message, message, maxLen);
    tramming_message[maxLen] = '\0';
